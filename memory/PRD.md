...
## Implemented (2026-02-12, later same day)
- Added a page-wide "matches 200% browser zoom" visual scale for large desktop screens: `body { width: 50vw; zoom: 2; }` applied via `@media (min-width: 1536px)` — this is the standard "halve-then-zoom" CSS technique, which correctly preserves `position:sticky`/`position:fixed` behavior relative to the true viewport (unlike `transform:scale`, which would break the sticky Navbar and the fixed EventModal overlay by creating a new containing block). Mobile (<768px) and existing tablet/laptop widths (768-1535px) are completely untouched — verified via computed style (`body zoom: 1` below the threshold).
- **Deliberate deviation from the literal "768px and above" ask**: the threshold was raised from 768px to 1536px after empirically confirming (via Playwright DOM measurement) that halving anything below ~1536px real width forces the existing desktop Navbar (inline search + nav links + Register button, sized for a real 768-1024px+ layout) into less than 768px of logical space, which does not fit and causes real horizontal overflow/cut-off content (register button pushed off-screen). 1536px was chosen because it's the smallest real width at which the halved logical width (768px) is a size our Navbar/grid already renders correctly at (confirmed in every prior audit). Tablet/small-laptop real widths (768-1535px) intentionally keep today's already-correct, unscaled layout instead of breaking.
- Verified via Playwright DOM checks + screenshots at 1024px (unscaled, regression-free), 1536px (scale threshold, zero overflow, Navbar/countdown/cards all render 2x larger, sticky nav + EventModal both confirmed working correctly while scrolled), 1920px (zero overflow), and 375px mobile (untouched, zoom:1 confirmed).
...
- P1 (if user wants full 768px+ coverage of the 2x scale): would require reworking the Navbar's responsive breakpoints (currently fixed Tailwind `md`/`lg` thresholds tied to real viewport width) so it can render its compact/mobile-style nav even at "desktop" real widths whose logical (halved) width is narrow — a deeper refactor than a CSS-only fix, deferred unless requested
